...
export default function WaterRoleScene({ onReady }) {
  ...
  const t = useSpring(dissolved ? 1 : 0, { stiffness: 0.12 });
    ...
    <div className={v.stage} style={{ width: '100%' }}>
      <div className={v.svgWrap} style={{ maxWidth: 340 }}>
        <svg viewBox="0 0 300 170" className={v.svg} role="img" aria-label={dissolved ? 'Salt dissolved into ions in water' : 'Salt crystal in water'}>
          ...
          {WATER.map((w, i) => (
            <g key={`w${i}`} style={{ transform: `translateY(${Math.sin((t + i) * 3) * 1.5}px)` }}>
          ...
          {IONS.map((ion) => {
            const x = lerp(ion.from[0], ion.to[0], t);
            const y = lerp(ion.from[1], ion.to[1], t);
            const r = 12;
            return (
              <g key={ion.id}>
